...
type<-patients[,c(2,8)]
type<-type[grep('1型',type$诊断名称),]
type$group<-ifelse(type$诊断名称 %in% c('1型糖尿病'),'No complication','Complication')
...
save(pie,file = 'diabetes-compication-plot-type1.rda')
ggsave('diabetes-complication-plot-type1.pdf',width = 6,height = 6)

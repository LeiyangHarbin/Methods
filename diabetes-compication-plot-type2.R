...
type<-patients[,c(2,8)]
type<-type[grep('2型',type$诊断名称),]
type$group<-ifelse(type$诊断名称 %in% c('2型糖尿病'),'No complication','Complication')
...
save(pie,file = 'diabetes-compication-plot-type2.rda')
ggsave('diabetes-complication-plot-type2.pdf',width = 6,height = 6)

# -*- coding: utf-8 -*-
"""
Created on Tue May  9 15:59:05 2023
...
#######调用数据
train=pd.read_csv('normal-diabetes-II-train.tsv',header='infer',sep="\t")
test=pd.read_csv('normal-diabetes-II-test.tsv',header='infer',sep="\t")
...
axes[0].savefig('normal-diabetes-II-train-ROC.pdf')
...
axes[0].savefig('normal-diabetes-II-test-ROC.pdf')
...
##########用1型糖尿病的数据测试
diabetes=pd.read_csv('D:/data/diabetes/diabetes-I/normal-diabetes-I.tsv',header='infer',sep="\t")
X_diabetes=diabetes.iloc[:,0:23]
y_diabetes=diabetes.iloc[:,23]
y_pre=xgb_model.predict(X_diabetes)
acc=accuracy_score(y_pre,y_diabetes)
f=open("diabetes-I accuracy score.txt","w")
f.write(str(acc))
f.close()

y_diabetes_prob=xgb_model.predict_proba(X_diabetes)
fpr,tpr,thresholds=roc_curve(y_diabetes,y_diabetes_prob[:,1])
roc_auc=auc(fpr,tpr)
...
axes[0].savefig('normal-diabetes-II-diabetes-I-ROC.pdf')

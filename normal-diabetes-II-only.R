...
setwd('D:/data/diabetes/diabetes-II/only')
diabetes_all<-diabetes_data[grep("2型",diabetes_data$诊断名称),]
normal<-na.omit(Blood_routine_masking)
normal<-normal[normal$白细胞数目!='****',]
table(diabetes_all$诊断名称)

diabetes_com<-diabetes_all[-which(diabetes_all$诊断名称=='2型糖尿病'),]
diabetes<-diabetes_all[which(diabetes_all$诊断名称=='2型糖尿病'),]
normal<-normal[sample(nrow(normal),nrow(diabetes1)),]
...
write.table(train,file = 'normal-diabetes-II-only-train.tsv',sep = '\t')
write.table(test,file = 'normal-diabetes-II-only-test.tsv',sep = '\t')

######并发症样本
save(diabetes_com,file = 'diabetes with complication.rda')
diabetes_com<-diabetes_com[,a]
colnames(diabetes_com)<-featurename[match(colnames(diabetes_com),featurename[,1]),2]
diabetes_com<-diabetes_com[,5:ncol(diabetes_com)]

diabetes_com$label<-1
write.table(diabetes_com,file = 'diabetes-with-complication-feature-label.tsv',sep = '\t')
